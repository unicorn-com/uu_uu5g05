...
try {
  ...
  jest.doMock("uu_appg01_oidc", () => {
    // NOTE uu_appg01_oidc >= 3.9.1 started to call /oidc/getClientId immediately during
    // library startup and it does it always, even in tests (where it fails). We'll set clientId which
    // will bypass it (until uu_appg01_oidc mocking / behavior in tests gets revised).
    let env = window.uu5Environment || window.UU5?.Environment; // eslint-disable-line no-undef
    if (!env) env = window.uu5Environment = {}; // eslint-disable-line no-undef
    env["uu_app_oidc_providers_oidcg02_client_id"] ??=
      "uu-oidc:unregistered-client:" + Math.random().toString(16).substring(2, 10);
    ...
    class MockAuthenticationService {
      ...
      static async authenticate() {
        ...
        oidcSession = _clone(oidcSession);
    ...
    class Session {
      ...
      async close() {
        this._identity = null;
        let newSession = _clone(this);
        newSession._identity = null;
        return MockAuthenticationService._triggerEvent("sessionChanged", newSession);
      }
      assertTrustiness() {}
      getAttribute(name) {
