...
const YEAR_FORMAT_REGEX = /^\d{4}$/;

class DateValue extends FormatValue {
  static VALID_CHARS = "Y";

  static parseValue(valueStr) {
    if (YEAR_FORMAT_REGEX.test(valueStr)) return valueStr;

    console.warn(`Invalid year "${valueStr}" to paste.`);
    return null;
  }

  static #splitValueToList(value) {
    return [...value.match(/(\d{4}|Y{4})/g)];
  }
  ...
  static #parseDateToObject(isoValue) {
    ...
    if (isoValue != undefined) {
      uuDate = new UuDate((isoValue + "").padStart(4, "0"));
      year = uuDate.getYear();
    }
  ...
  increase() {
    this._activeSlot.increase({ year: this._slotMap.year.getValue() });
    return this.getValue();
  }

  decrease() {
    this._activeSlot.decrease({ year: this._slotMap.year.getValue() });
    return this.getValue();
  }
  ...
  write(value) {
    ...
    this._activeSlot.setValue(+this._tempValue, { year: this._slotMap.year.getValue() });
    if (prevValue !== this._activeSlot.getValue()) {
      if (this._activeSlot.isMaxChars({ year: this._slotMap.year.getValue() })) this.selectNext();
      return this.getValue();
    }
  }
  ...
  toIsoString() {
    if (this.isCompleted()) {
      return new UuDate(this.getValue()).format(undefined, { format: "YYYY" });
    } else {
      return undefined;
    }
  }
  ...
  #isValidStep(valueUuDate) {
    ...
    const diff = (valueUuDate.getYear() * 12 - minUuDate.getYear() * 12) % step;
    return diff === 0;
  }
  ...
  #formatValueByFormat(value, format = "YYYY") {
