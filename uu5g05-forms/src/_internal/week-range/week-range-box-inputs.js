...
function getFormattedFormat(lang) {
  ...
  let formattedFormat = "WW YYYY";
  for (let key of ["year", "weekOfYear"]) {
    formattedFormat = formattedFormat.replace(new RegExp(`[${key[0].toUpperCase()}]+`), (v) => {
      let result = dn.of(key)[0].toUpperCase().repeat(v.length);

      if (key === "weekOfYear") {
        if (/^(cs|sk|uk)/.test(lang)) {
...
const WeekRangeBoxInputs = createVisualComponent({
  //@@viewOn:statics
  uu5Tag: Config.TAG + "WeekRangeBoxInputs",
  //@@viewOff:statics
...
//@@viewOn:exports
export { WeekRangeBoxInputs };
export default WeekRangeBoxInputs;
//@@viewOff:exports
