...
async function setup(props = {}, { Wrapper = Fragment } = {}) {
  const api = await InputComponent.setup(
    withInputController(Radios.Input),
    {
      ...getDefaultProps(),
      ...props,
    },
...
describe("Uu5Forms.Radios.Input", () => {
  ...
  it("checks itemList is properly shown", async () => {
    ...
    await user.click(input);
    const inputApple = Test.screen.getByRole("radio", { name: "Apple" });
    const inputOrange = Test.screen.getByRole("radio", { name: "Orange" });

    expect(inputApple).toBeInTheDocument();
    expect(inputOrange).toBeInTheDocument();
  });
  ...
  it("checks validateOnChange property is properly working", async () => {
    ...
    const radiosElement = Test.screen.getByText("No");
    await user.click(radiosElement);

    expect(handleValidationEnd).toBeCalledTimes(2);
    expect(handleValidationEnd).toHaveBeenLastCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ errorList: [] }),
      }),
    );
  });
