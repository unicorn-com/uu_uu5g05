...
import { testAutoFocus, testValidateOnMount, testValidationRef, withInputController } from "./internal/test-tools";
...
async function setup(props = {}, { Wrapper = Fragment } = {}) {
  const api = await FormInputComponent.setup(
    withInputController(Checkboxes),
    {
      ...getDefaultProps(),
      ...props,
    },
...
describe("Uu5Forms.Checkboxes", () => {
  ...
  it("checks itemList is properly shown", async () => {
    ...
    const checkboxInputDog = Test.screen.getByRole("checkbox", { name: "Apple" });
    const checkboxInputCat = Test.screen.getByRole("checkbox", { name: "Orange" });

    expect(checkboxInputDog).toBeInTheDocument();
    expect(checkboxInputCat).toBeInTheDocument();
  });
  ...
  it.each([
    ...
    expect(checkboxInput.parentElement).toHaveGdsShape(inputColorPath);
  });
