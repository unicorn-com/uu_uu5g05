...
async function updateExamples() {
  ...
  return batchedExec(
    Config.COMPONENTS.map((tagName) => () => {
      return new Component(tagName, Config.BOOK_AWID, Config.INCLUDE_NAMESPACE).update_examples();
    })
  );
}
...
async function updateSources() {
  ...
  return batchedExec(
    Config.COMPONENTS.map((tagName) => () => {
      return new Component(tagName, Config.BOOK_AWID, Config.INCLUDE_NAMESPACE).update_source();
    })
  );
}
